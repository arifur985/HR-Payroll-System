...
$results = mysqli_query($db, "SELECT MONTHNAME(date) as mname,request,disburse FROM employee where employee_email='$email'"); 
// echo $results;
...
<table>
    ...
    <?php while ($row = mysqli_fetch_array($results)) { ?>
        <tr>
            ...
            <?php if ($row['request']==1) { ?>
                <?php echo "requested"; ?>
            <?php } else { ?>
                <?php echo "not requested"; ?>
            <?php } ?>
                
            </td>
            <td>
                
            <?php if ($row['disburse']==1) { ?>
                <?php echo "Paid"; ?>
            <?php } else { ?>
                <?php echo "not paid"; ?>
            <?php } ?>
